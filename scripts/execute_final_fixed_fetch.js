...
async function runFinalFixedFetch() {
    console.log('--- Fetching Settings ---');
    const { data: settings, error: sErr } = await supabase.from('kts_settings').select('*');
    if (sErr || !settings) {
        console.error('Failed to load settings:', sErr?.message);
        return;
    }
    const config = settings.reduce((acc, s) => { acc[s.key] = s.value; return acc; }, {});

    if (config['GMAIL_IMAP_ENABLED'] !== 'true') {
        console.warn('GMAIL_IMAP_ENABLED is not true. Current value:', config['GMAIL_IMAP_ENABLED']);
        // Proceed anyway for testing if credentials exist
    }
    ...
    try {
        ...
        try {
            // Focus on all recent messages
            const messages = client.fetch({ since: new Date(Date.now() - 3600000) }, { envelope: true, uid: true, bodyStructure: true });

            for await (const msg of messages) {
                console.log(`Checking UID:${msg.uid} - "${msg.envelope.subject}"`);
                const attachments = findAttachments(msg.bodyStructure);
                
                for (const att of attachments) {
                    console.log(`   Found match: ${att.filename} (${att.mime})`);
                    ...
                    else console.log(`   ✅ Saved: ID ${data[0].id}, Invoice: ${record.invoice_number}, Status: ${record.status}`);
                }
            }

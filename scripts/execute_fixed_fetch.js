...
function parseInvoiceXML(content) {
    try {
        ...
        const payableAmount = invoice['cac:LegalMonetaryTotal']?.['cbc:PayableAmount'] || invoice['LegalMonetaryTotal']?.['PayableAmount'];
        let totalAmountValue = 0;
        if (payableAmount) {
            totalAmountValue = typeof payableAmount === 'object' ? (payableAmount['#text'] || payableAmount['content'] || 0) : payableAmount;
        }
...
async function runFixedFetch() {
    ...
    try {
        ...
        try {
            // Only focus on UID 10 and 11 for now to save time
            const lastMsgs = client.fetch({ uid: '10:*' }, { envelope: true, uid: true, bodyStructure: true });

            for await (const msg of lastMsgs) {
                console.log(`Processing UID:${msg.uid} - "${msg.envelope.subject}"`);
                const attachments = findAttachments(msg.bodyStructure);
                
                for (const att of attachments) {
                    console.log(`   Found: ${att.filename} (${att.mime})`);
                    // Use more reliable way to stream content
                    ...
                    let attachmentType = att.filename.endsWith('.pdf') ? 'pdf' : 'xml';
    ...
    } catch (err) { console.error('Error:', err); } finally { await client.logout(); }
}

...
async function debugRecipe() {
  ...
  // Check Usage Types
  const { data: usageTypes, error: utError } = await userClient
    .from("usage_types")
    .select("*");
  ...
  // Check Products
  const { data: products, error: prodError } = await userClient
    .from("products")
    .select("*")
    .limit(1);
  ...
  if (recipeError) {
    ...
    const { data: standards, error: stdError } = await userClient
      .from("compliance_standards")
      .select("*");
    if (stdError)
      console.error("Error reading compliance_standards:", stdError);
    ...
    if (standards && standards.length > 0) {
      ...
      const { error: checkError } = await userClient
        .from("compliance_checks")
        .insert(checkData);

      if (checkError) {
        console.error(
          "Compliance Check Insert FAILED (RLS issue likely):",
          checkError,
        );
      } else {

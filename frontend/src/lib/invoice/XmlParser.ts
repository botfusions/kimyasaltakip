...
export class XmlInvoiceParser {
  static parse(xmlContent: string): Invoice | null {
    try {
      ...
      const supplierName =
        supplierParty?.["cac:PartyName"]?.["cbc:Name"] || "Bilinmiyor";
      const supplierAddress =
        supplierParty?.["cac:PostalAddress"]?.["cbc:CityName"];

      // Extract buyer info (AccountingCustomerParty)
      const buyerParty = invoice["cac:AccountingCustomerParty"]?.["cac:Party"];
      const buyerName =
        buyerParty?.["cac:PartyName"]?.["cbc:Name"] || "Bilinmiyor";
      ...
      const lines: InvoiceLine[] = invoiceLines.map((line: any) => {
        ...
        const buyerCode =
          item["cac:BuyersItemIdentification"]?.["cbc:ID"] || "";
        ...
        const quantity = parseFloat(
          line["cbc:InvoicedQuantity"] || line["cbc:DeliveredQuantity"] || "0",
        );
        const unitPrice = parseFloat(
          line["cac:Price"]?.["cbc:PriceAmount"] || "0",
        );

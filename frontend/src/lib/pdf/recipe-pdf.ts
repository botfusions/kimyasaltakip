...
export const generateRecipePDF = (recipe: Recipe) => {
  ...
  if (recipe.approved_by_user) {
    ...
    doc.setTextColor(...Colors.LightText);
    doc.text(
      t(`Imza ID: ${recipe.approved_by_user.signature_id}`),
      120,
      signatureY + 12,
    );
    doc.text(
      t(
        `Onay Tarihi: ${new Date(recipe.approved_at || "").toLocaleString("tr-TR")}`,
      ),
      120,
      signatureY + 17,
    );
  ...
  doc.setTextColor(...Colors.LightText);
  doc.text(
    t("Bu belge sistem tarafindan otomatik olarak olusturulmustur."),
    105,
    285,
    { align: "center" },
  );

  ...
  try {
    ...
    if (existingUsageType) {
      ...
      if (usageTypeError || !newUsageType) {
        return NextResponse.json(
          { error: "Failed to create usage type", details: usageTypeError },
          { status: 500 },
        );
      }
    ...
    const materials = [
      {
        code: "KIM-001",
        name: "Isflatic (Wetting Agent)",
        unit: "lt",
        category: "Kimyasal",
      },
      {
        code: "KIM-045",
        name: "Kostik (Kireç Önleyici)",
        unit: "kg",
        category: "Kimyasal",
      },
      {
        code: "BOYA-102",
        name: "Reaktif Mavi R-Special",
        unit: "kg",
        category: "Boya",
      },
      {
        code: "BOYA-005",
        name: "Reaktif Kırmızı 3B",
        unit: "kg",
        category: "Boya",
      },
    ...
    for (const mat of materials) {
      ...
      if (!matId) {
        ...
        if (createError) {
          ...
          return NextResponse.json(
            {
              error: `Failed to create material ${mat.code}`,
              details: createError,
            },
            { status: 500 },
          );
    ...
    if (existingRecipe) {
      ...
      if (recipeError || !newRecipe) {
        return NextResponse.json(
          { error: "Failed to create recipe", details: recipeError },
          { status: 500 },
        );
      }
    ...
    for (const item of items) {
      ...
      if (itemError) {
        return NextResponse.json(
          {
            error: `Failed to create recipe item for ${item.code}`,
            details: itemError,
          },
          { status: 500 },
        );

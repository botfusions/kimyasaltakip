    ...
    try {
        ...
        if (existingStandard) {
            ...
            const { data: newStandard, error: standardError } = await supabase
                ...
                .insert({
                    name: 'ZDHC MRSL v3.1',
                    description: 'Zero Discharge of Hazardous Chemicals Manufacturing Restricted Substances List',
                    organization: 'ZDHC',
                    version: '3.1',
                    is_active: true
                })
            ...
            if (standardError || !newStandard) {
                return NextResponse.json({ error: 'Failed to create standard', details: standardError }, { status: 500 });
            }
        ...
        if (existingSubstance) {
            ...
            const { data: newSubstance, error: substanceError } = await supabase
                ...
                .insert({
                    ...
                    description: 'Restricted red dye due to potential hazardous byproducts',
            ...
            if (substanceError || !newSubstance) {
                return NextResponse.json({ error: 'Failed to create restricted substance', details: substanceError }, { status: 500 });
            }
        ...
        if (existingUsageType) {
            ...
            const { data: newUsageType, error: usageTypeError } = await supabase
                ...
                .select('id')
                .single();
            if (usageTypeError) return NextResponse.json({ error: 'Usage type error', details: usageTypeError });
        ...
        const materials = [
            ...
            { code: "BOY-002", name: "EUROCTIVE RED FTR", unit: "%", category: "Boya" }, // This one is restricted
            { code: "BOY-003", name: "EUROCTIVE BRILLIANT BLUE FTR", unit: "%", category: "Boya" },
            { code: "DIS-001", name: "Dispers Baskı Patı", unit: "g", category: "Kimyasal" },
            ...
            { code: "PH-005", name: "PH = 5.3 - 5.8", unit: "%", category: "Ph Düzenleyici" }
        ];
        ...
        if (existingProduct) {
            ...
            const { data: newProduct, error: productError } = await supabase
                ...
                .select('id')
                .single();
            if (productError) return NextResponse.json({ error: 'Product error', details: productError });
        ...
        if (recipeError) return NextResponse.json({ error: 'Recipe error', details: recipeError });

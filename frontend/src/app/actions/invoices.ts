 ...
 */
export async function importInvoiceFromOCR(
  ocrText: string,
  ...
  try {
    ...
    return {
      success: false,
      error: error.message || "Fatura içe aktarılamadı",
    };
  }
}
...
export async function deleteInvoiceImport(invoiceNumber: string) {
  try {
    ...
    if (fetchError) {
      return {
        success: false,
        error: "Fatura hareketleri yüklenemedi: " + fetchError.message,
      };
    }
    ...
    if (!movements || movements.length === 0) {
      return {
        success: false,
        error: "Bu fatura numarasına ait hareket bulunamadı",
      };
    }
    ...
    if (deleteError) {
      return {
        success: false,
        error: "Hareketler silinirken hata: " + deleteError.message,
      };
    }

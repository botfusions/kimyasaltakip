...
export async function processIncomingInvoice(id: string) {
  try {
    ...
    if (
      !parsedData &&
      invoice.attachment_content &&
      invoice.attachment_type === "xml"
    ) {
    ...
    if (!parsedData || !parsedData.lines || parsedData.lines.length === 0) {
      await supabase
        ...
        .update({
          status: "error",
          error_message: "Fatura parse edilemedi veya satır bulunamadı",
        })
        .eq("id", id);
...
export async function importIncomingInvoice(id: string) {
  try {
    ...
    if (!invoice.match_result || !Array.isArray(invoice.match_result)) {
      return {
        success: false,
        error: "Önce faturayı işleyin (malzeme eşleştirmesi yapılmamış)",
      };
    }
  ...
  try {
    ...
    if (!fetcher) {
      return {
        success: false,
        error:
          "Gmail IMAP yapılandırması eksik veya devre dışı. Ayarlar sayfasından GMAIL_IMAP ayarlarını aktif edin.",
      };
    }

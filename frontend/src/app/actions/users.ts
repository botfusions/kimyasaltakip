...
export async function getUsers(filters?: {
  ...
  let query = supabase
    .from("kts_users")
    .select(
      "id, email, name, role, is_active, phone, signature_id, created_at, last_login_at",
    )
...
export async function createUser(formData: FormData) {
  ...
  if (authError) {
    ...
    return {
      error: `Kullanıcı kaydı oluşturulurken hata: ${authError.message}`,
    };
  }
  ...
  if (error) {
    ...
    return {
      error: `Kullanıcı profili oluşturulurken hata oluştu: ${error.message}`,
    };
  }

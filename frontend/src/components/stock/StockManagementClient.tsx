...
export default function StockManagementClient({
  ...
  const filteredStocks = initialStocks.filter((stock) => {
    ...
    const matchesLowStock =
      !showLowStockOnly ||
      stock.quantity <= (stock.material?.critical_level || 0);
  ...
  const getStockStatus = (quantity: number, criticalLevel: number) => {
    if (quantity === 0)
      return { label: "Tükendi", color: "text-red-600 bg-red-50" };
    if (quantity <= criticalLevel)
      return { label: "Kritik", color: "text-orange-600 bg-orange-50" };
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-500 dark:text-gray-400">
                        {stock.material?.critical_level?.toLocaleString(
                          "tr-TR",
                        )}{" "}
                        {stock.material?.unit}
                      </td>

...
import { importInvoice, importInvoiceFromOCR } from '@/app/actions/invoices';
...
export default function InvoiceImportClient({ currentUser }: InvoiceImportClientProps) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow p-6">
                <h2 className="text-lg font-semibold text-gray-900 mb-4">XML Dosyası Yükle</h2>
            ...
            {result && result.unmatchedLines && result.unmatchedLines.length > 0 && (
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                    ...
                    <p className="text-sm text-orange-700 mb-3">
                        Aşağıdaki ürünler sistemdeki malzemelerle eşleştirilemedi. Lütfen malzeme kodlarını kontrol edin.
                    </p>
                    <div className="bg-white rounded border border-orange-200 overflow-hidden">
                        <table className="min-w-full divide-y divide-orange-200">
                            <thead className="bg-orange-100">
                                <tr>
                                    <th className="px-4 py-2 text-left text-xs font-medium text-orange-800">Kod</th>
                                    <th className="px-4 py-2 text-left text-xs font-medium text-orange-800">Ürün Adı</th>
                                    <th className="px-4 py-2 text-right text-xs font-medium text-orange-800">Miktar</th>
                                    <th className="px-4 py-2 text-right text-xs font-medium text-orange-800">Tutar</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-orange-100">
                                {result.unmatchedLines.map((line: InvoiceLine) => (
                                    <tr key={line.lineNumber}>
                                        <td className="px-4 py-2 text-sm text-gray-900">{line.productCode}</td>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
                <h3 className="text-sm font-medium text-gray-900 mb-2">Nasıl Kullanılır?</h3>
                <ol className="text-sm text-gray-700 space-y-2 list-decimal list-inside">
                    <li>Fatura dosyanızı seçin:
                        <ul className="ml-6 mt-1 text-xs text-gray-600 list-disc">
                            <li><strong>XML</strong>: E-Fatura (UBL-TR formatı) - En hızlı ve doğru</li>
                            <li><strong>PDF</strong>: OCR ile otomatik metin çıkarma</li>
                ...
                <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded">
                    <p className="text-xs text-blue-800">
                        <strong>💡 İpucu:</strong> En iyi sonuç için XML formatını kullanın. PDF ve görüntü dosyaları OCR ile işlenir ve %100 doğruluk garanti edilemez.
                    </p>
                </div>
            </div>
        </div>

...
export default function RecipeDetailsView({
  ...
  const handleStartProduction = async () => {
    if (!confirm("Bu reçeteyi üretime almak istediğinize emin misiniz?"))
      return;
  ...
  const totalQuantity = recipe.recipe_items.reduce(
    (sum, item) => sum + item.quantity,
    0,
  );

  const barcodeValue =
    recipe.barcode || recipe.order_code || recipe.version_code;
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <span
              className={`px-3 py-1 rounded-full text-sm font-semibold ${STATUS_LABELS[recipe.status]?.color}`}
            >
        ...
        <div className="space-y-6">
          ...
          {recipe.status === "approved" && recipe.approved_by_user && (
            <div className="bg-green-50 dark:bg-green-900/10 border border-green-100 dark:border-green-900/30 rounded-lg shadow-sm p-6">
              <h2 className="text-sm font-semibold text-green-700 dark:text-green-400 uppercase tracking-wider mb-4 underline decoration-2 decoration-green-300 underline-offset-4">
                ✓ Onaylayan (Dijital İmza)
              </h2>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <div className="font-semibold text-gray-900 dark:text-white">
                    {recipe.approved_by_user.name}
                  </div>
                  <div className="text-xs text-gray-500">
                    {recipe.approved_by_user.email}
                  </div>
                </div>
              </div>
              <div className="space-y-2 mt-4 pt-4 border-t border-green-200 dark:border-green-800/50">
                <div className="flex justify-between text-xs">
                  ...
                  <span className="font-mono font-bold text-green-800 dark:text-green-200">
                    {recipe.approved_by_user.signature_id.substring(0, 2)}**
                    {recipe.approved_by_user.signature_id.substring(4)}
                  </span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-green-700 dark:text-green-400 font-medium">
              ...
              <div className="mt-4 p-2 bg-white/50 dark:bg-black/20 rounded text-[10px] text-green-700 dark:text-green-400 italic text-center">
                ISO 9001:2015 Kalite Standartlarına Uygun Olarak Dijital Olarak
                Onaylanmıştır.
              </div>
            </div>
          )}

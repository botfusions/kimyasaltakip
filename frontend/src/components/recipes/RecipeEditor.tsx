...
import { createRecipe, updateRecipe, getRecipeById, submitForApproval } from '@/app/actions/recipes';
        ...
        try {
            ...
            if (result.error) {
                ...
                if (result.compliance && result.compliance.status === 'fail') {
                    ...
                    if (result.data?.id) {
                        try {
                            const approvalResult = await submitForApproval(result.data.id, JSON.stringify(result.compliance));
                            if (approvalResult.success) {
                                // Success!
                                alert('Reçete MRLS kontrolünden geçti ve yönetici onayına gönderildi.');
                                router.push('/dashboard/recipes');
                            } else {
                                setError('MRLS başarılı ancak onaya gönderilirken hata oluştu: ' + approvalResult.error);
                            }
                        } catch (submitErr) {

...
export default function RecipeEditor({ products, recipeId }: Props) {
  ...
  const loadRecipe = async () => {
    ...
    if (result.error) {
      ...
      setBatchRatio(
        result.data.batch_ratio ? String(result.data.batch_ratio) : "",
      );
      setProcessWashCount(
        result.data.process_wash_count
          ? String(result.data.process_wash_count)
          : "",
      );
      setCauldronQuantity(
        result.data.cauldron_quantity
          ? String(result.data.cauldron_quantity)
          : "",
      );
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // versionCode is auto-generated by backend if not provided
    ...
    if (processWashCount)
      formData.append("process_wash_count", processWashCount);
    if (cauldronQuantity)
      formData.append("cauldron_quantity", cauldronQuantity);
    ...
    try {
      ...
      if (result.error) {
        ...
        // If compliance issues found, show modal
        if (result.compliance && result.compliance.status === "fail") {
          setComplianceResult(result.compliance);
          setShowComplianceModal(true);
        } else {
          // Compliance passed, auto-submit for approval
          if (result.data?.id) {
            try {
              ...
              if (approvalResult.success) {
                // Success!
                alert(
                  "Reçete MRLS kontrolünden geçti ve yönetici onayına gönderildi.",
                );
                router.push("/dashboard/recipes");
              } else {
                setError(
                  "MRLS başarılı ancak onaya gönderilirken hata oluştu: " +
                    approvalResult.error,
                );
              }
            } catch (submitErr) {
              setError("Onaya gönderme hatası.");
            }
          } else {
            router.push("/dashboard/recipes");
          }
  ...
  const handleComplianceModalClose = () => {
    ...
    router.push("/dashboard/recipes"); // Default action on close
  };
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
          ...
          {items.length === 0 ? (
            <div className="text-center py-8 text-gray-500 dark:text-gray-400">
              Henüz malzeme eklenmedi. &quot;Yeni Satır Ekle&quot; butonunu
              kullanarak malzeme ekleyebilirsiniz.
            </div>
          ) : (
        ...
        <div className="flex gap-4">
          ...
          <Button
            ...
            className="flex-1"
          >
            {recipeId ? "Kontrol Et ve Güncelle" : "Sistem Kontrolü ve Ön Onay"}
          </Button>
        </div>
      </form>
      ...
      <Modal
        ...
        title="⚠️ Uygunluk Kontrolü Uyarısı"
        size="lg"
      >
        <div className="space-y-4">
          <p className="text-gray-700 dark:text-gray-300">
            Reçete oluşturuldu ancak aşağıdaki kısıtlı maddeler tespit edildi:
          </p>

          {complianceResult && complianceResult.violations && (
            <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg overflow-hidden">
              <table className="w-full text-sm text-left">
                <thead className="bg-red-100 dark:bg-red-900/40 text-red-900 dark:text-red-100">
                  <tr>
                    <th className="p-3">Standart</th>
                    <th className="p-3">Malzeme</th>
                    <th className="p-3">CAS No</th>
                ...
                <tbody className="divide-y divide-red-200 dark:divide-red-800">
                  {complianceResult.violations.map((v: any, i: number) => (
                    <tr key={i}>
                      <td className="p-3 font-medium">{v.standard}</td>
                      <td className="p-3">{v.material}</td>
                      <td className="p-3 font-mono text-xs">{v.cas}</td>

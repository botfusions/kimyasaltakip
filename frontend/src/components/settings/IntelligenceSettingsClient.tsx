        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-sm text-gray-500">
                        AI Uzman Botu'nun canlı olarak takip edeceği dış bilgi kaynaklarını yönetin.
                    </p>
                </div>
                <Button onClick={() => setShowAddForm(!showAddForm)} variant={showAddForm ? "ghost" : "primary"} size="sm">
                    {showAddForm ? "Vazgeç" : <><Plus className="w-4 h-4 mr-2" /> Yeni Kaynak Ekle</>}
                </Button>
            </div>

            {showAddForm && (
                <form onSubmit={handleAdd} className="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900 p-6 rounded-2xl animate-in fade-in slide-in-from-top-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label className="block text-xs font-medium text-gray-500 mb-1">Kaynak Adı</label>
                            <input 
                                type="text" 
                                value={newName}
                        ...
                        <div className="md:col-span-2">
                            <label className="block text-xs font-medium text-gray-500 mb-1">URL (Crawl edilecek adres)</label>
                            <input 
                                type="url" 
                                value={newUrl}
                        ...
                        <div className="md:col-span-2 flex items-end">
                            <Button type="submit" variant="primary" className="w-full" disabled={isSaving}>
                                {isSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : "Kaynağı Tanımla ve Takibe Başla"}
                            </Button>
                        </div>
                    </div>
                </form>
                ...
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                        {sources.length === 0 ? (
                            ...
                            sources.map((source) => (
                                <tr key={source.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-900/30 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className={`p-2 rounded-lg ${source.is_active ? 'bg-blue-50 text-blue-600' : 'bg-gray-50 text-gray-400'}`}>
                                            ...
                                            <div>
                                                ...
                                                <a href={source.url} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-500 hover:underline flex items-center gap-1">
                                                    {source.url.substring(0, 40)}... <ExternalLink className="w-3 h-3" />
                                                </a>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        ...
                                        >
                                            {source.is_active ? <CheckCircle2 className="w-3 h-3 mr-1" /> : <XCircle className="w-3 h-3 mr-1" />}
                                            {source.is_active ? 'Aktif' : 'Pasif'}
                                        </button>
                                    </td>
                                    <td className="px-6 py-4 text-xs text-gray-500">
                                        {source.last_synced_at ? new Date(source.last_synced_at).toLocaleString('tr-TR') : 'Hiç taranmadı'}
                                    </td>
                                    <td className="px-6 py-4 text-right space-x-2">
                                        ...
                                        >
                                            {isSyncing === source.id ? <RefreshCw className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}
                                        </Button>
                                        <Button variant="ghost" size="sm" onClick={() => handleDelete(source.id)} className="text-red-500">
            ...
            <div className="p-4 bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-100 dark:border-yellow-900/30 rounded-xl flex gap-3">
                ...
                <p className="text-xs text-yellow-800 dark:text-yellow-200">
                    <strong>Crawl4AI Notu:</strong> Dış kaynaklar Markdown formatına dönüştürülerek vektör veritabanına kaydedilir. 
                    AI Uzman Botu bu verileri "Long-term Knowledge" olarak kullanır. Tarama işlemi kaynak yoğunluğu nedeniyle arka planda yürütülür.
                </p>
            </div>
        </div>
    );

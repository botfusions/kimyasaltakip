...
if (!SUPABASE_SERVICE_ROLE_KEY) {
    ...
    console.error('  SET SUPABASE_SERVICE_ROLE_KEY=your_service_role_key && node scripts/import-chemicals.js');
...
const JSON_FILE_PATH = path.join(__dirname, '../../docs/extract-data-2026-02-02.json');
...
async function importData() {
    ...
    try {
        ...
        if (!products || !Array.isArray(products)) {
            throw new Error('Invalid JSON format: "powerbi_products" array not found.');
        }
        ...
        for (let i = 0; i < products.length; i += BATCH_SIZE) {
            ...
            if (error) {
                ...
                process.stdout.write(`\rImported ${processedCount} / ${products.length} products...`);
            }
        }

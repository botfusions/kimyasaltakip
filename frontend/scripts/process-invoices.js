...
if (!supabase) {
  console.log(
    "NOTICE: SUPABASE_SERVICE_ROLE_KEY not provided. Running in DRY-RUN mode (Extraction only).",
  );
}
...
function parseInvoiceXML(xmlContent) {
  try {
    ...
    const lines = invoiceLines.map((line) => ({
      ...
      currency:
        line["cbc:LineExtensionAmount"]?.["@_currencyID"] || currencyCode,
    }));
...
function parseOCRText(ocrText) {
  try {
    ...
    const invMatch =
      ocrText.match(
        /(?:fatura|FATURA|irsaliye|İRSALİYE|IRSALIYE)\s*(?:no|NO)?\s*:?\s*([A-Z0-9-]{10,})/i,
      ) || ocrText.match(/([A-Z]{3}\d{13,})/);
    ...
    const supplierMatch = ocrText.match(
    ...
    const pattern =
      /([A-Z0-9-]{3,})\s+(.+?)\s+([\d.,]+)\s*(?:KG|kg|ADET|adt|L|lt)?\s+([\d.,]+)/g;
    let match;
    let lineNo = 1;
...
function matchMaterial(line, materials) {
  ...
  for (const m of materials) {
    ...
    if (pName && mName && pName.includes(mName))
      return { id: m.id, confidence: 0.8 };
  }
  ...
  try {
    ...
    if (ext === ".xml") {
      ...
      try {
        ...
        const ocrText = execSync(`python "${ocrScriptPath}" "${filePath}"`, {
          encoding: "utf-8",
        });
    ...
    console.log(
      `Extracted Invoice: ${invoiceData.invoiceNumber} from ${invoiceData.supplier}`,
    );
    console.log(`Found ${invoiceData.lines.length} lines.`);

    // SQL Generation Logic
    if (invoiceData && invoiceData.lines && invoiceData.lines.length > 0) {
      const supplierName = invoiceData.invoiceNumber.startsWith("OCR")
        ? "Bilinmeyen Tedarikçi"
        : "RUDOLF DURANER KİMYEVİ MADDELER TİC. ve SAN. A.Ş."; // Hardcoded for example
      ...
      invoiceData.lines.forEach((line) => {
        ...
        const cleanName = (line.productName || "Bilinmeyen Ürün").replace(
          /'/g,
          "''",
        );
...
(async () => {
  ...
  const files = [
    path.join(
      __dirname,
      "../../fatura/RUD2025000017302-D5366353-0A32-44C1-95E8-DCDE9EF5755C.xml",
    ),

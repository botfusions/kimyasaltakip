...
if (supabaseKey) console.log("Service Key found (starts with):", supabaseKey.substring(0, 10) + "...");
...
async function importData() {
    ...
    // Data to Import
    const materials = [
        { code: '33905C', name: 'RUCO-FLOW BBA', unit: 'kg', category: 'Kimyasal', is_active: true, critical_level: 100 },
        { code: '25-0035', name: 'RUCOGEN PVN', unit: 'kg', category: 'Kimyasal', is_active: true, critical_level: 500 }
    ];

    const stockMovements = [
        { code: '33905C', qty: 100, supplier: 'RUDOLF DURANER KİMYEVİ MADDELER TİC. ve SAN. A.Ş.', ref: 'RUD2025000017302', notes: 'Örnek Fatura: RUD2025000017302 (XML)' },
        { code: '25-0035', qty: 1000, supplier: 'RUDOLF DURANER KİMYEVİ MADDELER TİC. ve SAN. A.Ş.', ref: '7350213672', notes: 'Örnek Fatura: 7350213672 (PDF)' }
    ];
    ...
    for (const mov of stockMovements) {
        // Get Material ID
        const { data: matData } = await supabase.from('materials').select('id').eq('code', mov.code).single();
